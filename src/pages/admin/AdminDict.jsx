  ...
  return (
    <Layout>
      <div className="mb-4 sm:mb-6">
        ...
        <p className="text-gray-500 text-xs sm:text-sm mt-1">총 {total.toLocaleString()}개 코드</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="text-right px-4 py-3 text-gray-500 font-medium w-28">액션</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td colSpan={6} className="text-center py-10 text-gray-400 animate-pulse">로딩 중...</td>
              </tr>
            ) : items.length === 0 ? (
              <tr>
                <td colSpan={6} className="text-center py-10 text-gray-400">검색 결과가 없습니다.</td>
              </tr>
            ) : items.map((item) => (
              <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${item.is_hidden ? 'opacity-50' : ''}`}>
                <td className="px-4 py-2.5 font-mono text-gray-800">{item.code}</td>
                ...
                <td className="px-4 py-2.5 text-gray-400 hidden md:table-cell">{item.category}</td>
          ...
          <div className="flex items-center justify-between py-3">
            ...
            <div className="flex gap-1">
              <button
                onClick={() => setPage((p) => Math.max(0, p - 1))}
                disabled={page === 0}
                className="px-3 py-1.5 text-xs border border-gray-300 rounded disabled:opacity-40"
              >
                이전
              </button>
              <button
